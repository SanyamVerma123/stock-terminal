...
import { SECTOR_KEYS, sectorLabel } from "@/lib/markets";
...
export function SectorPerformanceHeatmap() {
  ...
  const symbols = resolved.map(({ value }) => {
    const column = value?.topCompanies.columns.find((item) => /symbol|ticker|code/i.test(item));
    return column ? value?.topCompanies.rows[0]?.[column] : undefined;
