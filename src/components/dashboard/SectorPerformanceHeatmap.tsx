...
export function SectorPerformanceHeatmap() {
  ...
  return <section className="rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-primary">Sector performance map</p><h2 className="mt-1 text-base font-semibold text-foreground">Market direction by sector</h2><p className="mt-1 max-w-3xl text-xs leading-5 text-muted-foreground">A packed square map: color shows the 1D direction and tile scale distinguishes the largest sector groups.</p></div><span className="rounded-full border border-border bg-muted/30 px-2.5 py-1 text-[10px] font-medium text-muted-foreground">{ready}/{SECTOR_KEYS.length}</span></div><div className="mt-4 grid aspect-square grid-cols-6 grid-rows-6 gap-1.5">{resolved.map(({ sectorKey, value }, index) => { const signal = signals.find((item) => item.sectorKey === sectorKey); const move = signal?.symbol ? quoteBySymbol.get(signal.symbol)?.changePercent ?? null : null; const size = index < 2 ? "col-span-3 row-span-2" : index < 5 ? "col-span-2 row-span-2" : "col-span-2"; const compact = index >= 5; const detail = signal?.label ?? (signal?.symbol ? "Lead-company proxy" : value?.marketCap ? fmtCompact(value.marketCap) : "Awaiting data"); return <article key={sectorKey} title={sectorLabel(sectorKey)} className={`min-w-0 overflow-hidden rounded-lg border p-1.5 transition-transform hover:-translate-y-0.5 sm:p-2 ${size} ${movementClass(move)}`}><p className={`truncate font-semibold leading-tight ${compact ? "text-[7px] sm:text-[9px]" : "text-[9px] sm:text-[11px]"}`}>{sectorLabel(sectorKey)}</p><p className={`mt-1 truncate tabular font-semibold ${compact ? "text-[10px] sm:text-xs" : "text-sm sm:text-lg"}`}>{move === null ? "—" : `${move >= 0 ? "+" : ""}${move.toFixed(2)}%`}</p>{!compact && <p className="mt-1 truncate text-[8px] opacity-75 sm:text-[10px]">{detail}</p>}</article>; })}</div></section>;
}
