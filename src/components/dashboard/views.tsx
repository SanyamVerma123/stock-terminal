...
import { ArrowDown, ArrowUp, Bell, BellRing, FolderPlus, Inbox, KeyRound, MailCheck, ShieldCheck, Star, Trash2 } from "lucide-react";
        ...
        {cloudAccount ? (
          <div className="mt-4 space-y-3 rounded-xl border border-border/60 bg-background/25 p-3">
            ...
            <div className="flex flex-wrap items-center gap-2 border-t border-border/60 pt-3">
              <span className={cn("inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px]", cloudAccount.emailVerified ? "border-positive/30 bg-positive/10 text-positive" : "border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-300")}>
                {cloudAccount.emailVerified ? <MailCheck className="h-3.5 w-3.5" /> : <ShieldCheck className="h-3.5 w-3.5" />}
                {cloudAccount.emailVerified ? "Email verified" : "Email not verified"}
              </span>
              {!cloudAccount.emailVerified ? <button type="button" onClick={() => void postCloudAuth({ action: "send_verification" }).then(() => setAccountMessage("Verification link sent. Check your inbox."), (error: unknown) => setAccountMessage(error instanceof Error ? error.message : "Unable to send verification email."))} className="h-8 rounded-lg border border-primary/40 px-3 text-xs text-primary">Send verification link</button> : null}
          ...
          </form> : <form onSubmit={(event) => { event.preventDefault(); void signIntoCloud({ mode: accountMode, email: accountEmail, password: accountPassword, ...(accountMode === "register" && accountName.trim() ? { displayName: accountName } : {}) }).then((success) => { if (success) { setAccountPassword(""); if (accountMode === "register") setAccountMessage("Account created. Check your inbox to verify this email."); } }); }} className="mt-4 grid gap-2 md:grid-cols-2">
