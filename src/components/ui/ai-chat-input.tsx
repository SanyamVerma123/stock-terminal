...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    ...
    const handleSubmit = () => {
      if (isStreaming || (value.trim() === "" && !hasAttachments)) return;
      setIsSmoothResize(false);
      onSubmit?.(value, {
        model: selectedModel,
        effort: efforts[effortIndex] ?? efforts[0] ?? "Medium Effort",
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <DynamicBarsIcon level={efforts[effortIndex] ?? efforts[0] ?? "Medium Effort"} />
                <span className="text-xs font-semibold select-none transition-colors">
                  <MorphingText text={efforts[effortIndex] ?? efforts[0] ?? "Medium Effort"} />
                </span>
              </button>
